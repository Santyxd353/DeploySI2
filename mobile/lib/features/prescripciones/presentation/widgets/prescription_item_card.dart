...
class PrescriptionItemCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final subtitleParts = <String>[
      if (item.detectedQuantity.isNotEmpty) 'Cantidad: ${item.detectedQuantity}',
      if (item.detectedInstructions.isNotEmpty) 'Dosis diaria: ${item.detectedInstructions}',
      if (item.detectedDuration.isNotEmpty) 'Días: ${item.detectedDuration}',
    ];

    return Card(
      child: ListTile(
        title: Text(item.resolvedName.isNotEmpty ? item.resolvedName : item.detectedName),
