...
class PrescriptionService {
  PrescriptionService({ApiClient? apiClient})
  ...
  Future<PrescriptionCapture> getCapture(
    int captureId, {
    String? accessToken,
  }) async {
  ...
  Future<Map<String, dynamic>> updateCaptureItem({
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw PrescriptionServiceException(
        _extractDetail(
          data,
          fallback: 'No se pudo actualizar el item de receta.',
        ),
      );
    }
    return data;
  ...
  Future<SavedPrescription> getSavedRecipe(
    int recipeId, {
    String? accessToken,
  }) async {
    final response = await _apiClient.get(
  ...
  Future<SavedPrescription> updateSavedRecipe({
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw PrescriptionServiceException(
        _extractDetail(
          data,
          fallback: 'No se pudo actualizar la receta guardada.',
        ),
      );
    }
    return SavedPrescription.fromJson(data);
